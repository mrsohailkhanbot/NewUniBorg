...
async def _(event):
    ...
    if not borg.storage.USER_AFK:  # pylint:disable=E0602
        ...
        if reason:
            await event.edit(f"😴😴😴😴😴😴😴\n**I'm Offline Ek baar pdh le fir msg mt kriyon bsdk**\n**Shant Rhe Aakr Baat Krta Hu:** {reason}\n😒😒😒😒😒😒😒")
        else:
...
async def on_afk(event):
    ...
    if borg.storage.USER_AFK and not (await event.get_sender()).bot:  # pylint:disable=E0602
        ...
        msg = None
        message_to_reply = f"😴😴😴😴😴😴😴\n**Bsdk Ruk Ja Abhi AFK hu (Away From Keyboard)*\n**Sale Ruk Ja Reason Pdh Le Offline Ka Bsdk:** {reason}\n😴😴😴😴😴😴😴" \
            if reason \
